...
- 浏览器会弹一次「保存文件」框，选个位置保存 **`log.txt`**（建议就存下载文件夹）。
  选好后日志会**实时持续写入**这个文件——每次请求的网址/状态、每章正文、每张插图、
  429 限流冷却、网盘上传分块都记下来。**即使进度条卡住，日志仍在继续写**（卡住的只是
  某个等待中的请求，页面定时刷盘照常工作），所以能精确看到卡在**哪一章 / 哪张图 / 哪个网址**。
- 若浏览器不支持选文件（或你点了取消），日志会先存在内存里，**下载结束时自动存成 `log.txt`**
  到下载目录。
- 把 `log.txt` 发给我，就能定位卡住原因。用完取消勾选即可（平时不用开）。
