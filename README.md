...
>
> **关于第 2 步「解析」时 macOS 弹出的权限提示**：程序过 Cloudflare 需要启动你本机的 Chrome，
> 而这个 App 没有做苹果签名/公证，所以 macOS 会弹一次「是否允许本程序访问/控制其他 App」的隐私提示
> —— **点「允许」即可，只需确认这一次**，之后解析不再弹。若手滑点了「拒绝」导致后续连不上浏览器，
> 到「系统设置 → 隐私与安全性 →『App 管理』（或『自动化』）」里把 **Bilimanga-Downloader** 放行即可。
> （这不影响下载本身，只是 macOS 对未公证 App 的例行确认。）
...
除漫画外，也支持下载 [linovelib](https://www.linovelib.com) 的**轻小说**，打包为文字型 EPUB
（正文 + 内嵌插图，自动做字体反混淆）。用法：**粘贴轻小说链接**（形如
`https://www.linovelib.com/novel/2139.html`）点「解析」即可，程序按链接自动识别漫画/轻小说。

> ⚠️ linovelib 现在**需要登录**才能读取正文。首次下载轻小说前，请到「设置 → 登录轻小说」点一次，
> 会弹出一个浏览器窗口让你登录；登录状态会被记住（写入独立浏览器资料目录），之后下载**全程无需再登录、也不再弹窗**。
> 漫画（bilimanga）无需登录。
